 ...
 * (c) 2013 Adam Shaw
 * 
 * This file modified by NextSphere based on requirements
 */
...
var langOptionHash = {
	en: {
		columnFormat: {
			week: 'ddd D' // override for english. different from the generated default, which is MM/DD
		},
...
function Calendar(element, instanceOptions) {
	...
	/*Modified by the NextSphere*/	
	function changeView(viewName) {
		var courseStatus = $('#courseStatus').val();//UNPUBLISHED
		var associationType = $('#associationType').val();//COURSE,GROUP
		var loggedInUserRoleName = $('#loggedInUserRoleName').val();//PUBLISHER
		
		if(viewName == 'agendaDay' ){
			$("#calendar").css({"width":"64%","float":"right"});
			$("#miniCalendarContainer").removeClass('hide');
			$("#categoriesMenuDiv").addClass('hide');
			
			if(associationType == 'COURSE')
				$('#courseinviteefilleroptions').addClass('hide');
			 
		}else{
			$("#miniCalendarContainer").addClass('hide');
			$("#calendar").css({"width":"553px"});
			$("#categoriesMenuDiv").removeClass('hide');
			if(associationType == 'COURSE' && (loggedInUserRoleName == 'PUBLISHER' || loggedInUserRoleName =='ADMINISTRATOR'))
				$('#courseinviteefilleroptions').removeClass('hide');
			...
			if (
				...
				if (elementVisible()) {
					...
					var defaultCategories = [];
	            	  ...
	            	  $.each($("[id^=eventCategoryCheckBox]"),function(index,value){
	            		  if($(value).is(":checked")){
	            			  if($(value).val() != ""){
	            				  var isCustomCategory = $('#filterCalendarCategoryDiv_'+$(value).val()).attr("isCustomCategory");
	            				  if(isCustomCategory == 'true'){
	            					  categories.push($(value).val());  
	            				  }else{
	            					  defaultCategories.push($('#filterCalendarCategoryDiv_'+$(value).val()).attr("categoryType"));
	            				  }
	            	  ...
	            	  var view = $('#calendar').fullCalendar('getView');
	            	  	...
	            	  	/****************Below Code Will consider the date according to the Time Zone*************************/
	            	  var tempDate;
	            	  var monthViewStartDate,monthViewEndDate;
	            	     if(view.name == 'month'){
	            	    	 //alert(new Date(view.start)+'------------'+new Date(view.end));
	            	    	 monthViewStartDate = new Date(view.start);
	            	    	 monthViewEndDate = new Date(view.end);
	            	    	 monthViewStartDate = FullCalendarWidget.formatDate_yyyymmdd(monthViewStartDate);
	            	    	 monthViewEndDate = FullCalendarWidget.formatDate_yyyymmdd(monthViewEndDate);
	            	    	 tempDate = new Date(date.clone().stripTime().startOf('month'));
	            	     }else if(view.name == 'agendaWeek'){
	            	    	 tempDate = new Date(date.clone().stripTime().startOf('agendaWeek'));
	            	     }else if(view.name == 'agendaDay'){
	            	    	 tempDate = new Date(date.clone().stripTime().startOf('agendaDay'));
	            	     }
	            	   
	            	    var newDate = new Date(tempDate.getTime()+tempDate.getTimezoneOffset()*60*1000);

	            	    var offset = tempDate.getTimezoneOffset() / 60;
	            	    var hours = tempDate.getHours();
	            	    newDate.setHours(hours - offset);
	            	    /*Modified by the NextSphere*/
	            	    /****************Above Code Will consider the date according to the Time Zone*************************/
	            	    var calOptions = FullCalendarWidget.loadEventsRequest(view.name,newDate);
	            	    ...
	            	    calOptions.requestInfo['callBack'] = FullCalendarWidget.renderFullCalendarEvents;
	  					...
	  					for(var i=0;i<defaultCategories.length;i++){
	  						eventSearchCriteria.push({
	  							...
	  							"eventSearchCriteriaValue": defaultCategories[i]
	  						});
  					}
	  					
	  					var associationType = $('#associationType').val();//COURSE,GROUP
	  					var loggedInUserRoleName = $('#loggedInUserRoleName').val();//PUBLISHER
	  					//checking if course calendar view or not and building the senders type check boxes to filter on the calaender events.
	  					if(associationType == 'COURSE' && (loggedInUserRoleName == 'PUBLISHER'  || loggedInUserRoleName =='ADMINISTRATOR')){
		  					var object = $('.coursecalendersenderstype:visible');
		  					//looping with all event privacies check boxes and preparing search criteria object for request.
		  					for(var i = 0; i<object.length;i++){
						    	if(object[i].checked){
							    	if(object[i].value == 'ALUMNI'){
							    		eventSearchCriteria.push({
				  							"eventSearchCriteria": "EVENTPRIVACY",
				  							"eventSearchCriteriaValue": object[i].value
				  						});
...
function Header(calendar, options) {
	...
	/*
	 * @NextSphere
	 * This function is modified by NextSphere based on requirements and behaviour of the full calendar.*/
	function render() {
		var sections = options.header;

		tm = options.theme ? 'ui' : 'fc';
       var categories = options.categories;
       if(categories && categories.length == undefined){
    	   categories = [categories];
       }
		if (sections) {
			var categoryhtml = '<span class="dropdown grey-dropdown-panel" id="calendar-white-dropdown-panel">'
				+'	<a href="javascript:void(0);" id="categoryMenuAnchor" data-toggle="dropdown" class="dropdown-toggle"><span class="tag-sm-icons" id="categorydropdownidd"></span> </a>'
				+'	<ul class="dropdown-menu arrow-right white-dropdown min-width-200 hide" id="categoriesCalendarUL">'
				+'		<div class="font-14px text-left category-dp-heading">Category</div>'
				+'		<div id="calendarCategoriesScorllDiv" class="radiobuttons pad-left-15 float-left">';
			categoryhtml+='			<div class="text-left" id="filterCategoryAllDiv"><input class="css-checkbox" type="checkbox" id="eventCategoryCheckBox" value="" checked><label for="eventCategoryCheckBox" class="css-label pad-left"><pre>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;All</pre></label></div>';
			if(categories){
				for(var i=0;i<categories.length;i++){
					  categories[i].tooltipcategoryName = categories[i].categoryName;
					  categories[i].categoryName = (categories[i].categoryName.length > 20)?categories[i].categoryName.substr(0,20)+'..':categories[i].categoryName;
					  categoryhtml+='<div id="filterCalendarCategoryDiv_'+categories[i].categoryId+'" categoryId="'+categories[i].categoryId+'" '
					  +'isCustomCategory="'+categories[i].isCustomCategory+'" categoryColorCode="'+categories[i].categoryColorCode+'" '
					  +'categoryType="'+categories[i].categoryType+'" categoryName="'+categories[i].categoryName+'" class="text-left calendarCategories">'
					  +'<input class="css-checkbox" type="checkbox" id="eventCategoryCheckBox_'+categories[i].categoryId+'" value="'+categories[i].categoryId+'">'
					  +'<label for="eventCategoryCheckBox_'+categories[i].categoryId+'" isCustomCategory="'+categories[i].isCustomCategory+'" '
					  +'categoryColorCode="'+categories[i].categoryColorCode+'" class="css-label mediumblue" style="color:'+categories[i].categoryColorCode+'"'
					  +' title="'+categories[i].tooltipcategoryName+'">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<pre>'+categories[i].categoryName+'</pre></label>'
					  +'</div>';
				}
			}
			categoryhtml+='<div id="newCalendarCustomCategoryDiv"  class="text-left pad-bot-7">';
				if(options.isEditable == 'true'){
			categoryhtml+='<a href="javascript:void(0);" id="newCategoryEvent" ><span class="lightblue">New</span></a>';
				}else{
			//categoryhtml+='<a href="javascript:void(0);" class="ancher_lock lightblue" id="newCategoryEvent">New</a>';		
				}
				
			categoryhtml+='</div>'
				+'		</div>'
				+'	</ul>'
				+'  <div id="manageCalendarCategoriesDiv" class=""></div>'
				+'</span>';
			
			var categoryEle = $(categoryhtml).mouseleave(function(){
				//$(".white-dropdown-panel span").removeClass('selected');
				allowClose = true;
			});
			...
			var associationType = $('#associationType').val();//COURSE,GROUP
			var loggedInUserRoleName = $('#loggedInUserRoleName').val();//PUBLISHER
			var loggedInUserCircle = $('#loggedInUserCircle').val();//Educator
			//checking if course calendar view or not and building the senders type check boxes to filter on the calaender events.
			if(associationType == 'COURSE' && (loggedInUserRoleName == 'PUBLISHER'  || loggedInUserRoleName =='ADMINISTRATOR')){
			myvar = '<div class="horizontal-fields" id="courseinviteefilleroptions">'+
			'<input id="allcalonly" class="css-checkbox coursecalendersenderstype" checked="checked" type="checkbox" value="ALL">'+
			'<label class="css-label pad-left-30 senderseventtype-fontsize" for="allcalonly">All</label>'+
			'<input id="instructorscalonly" class="css-checkbox coursecalendersenderstype" type="checkbox" value="INSTRUCTOR">'+
			'<label class="css-label pad-left-30 senderseventtype-fontsize" for="instructorscalonly">Instructor</label>';
				if(courseStatus == 'PUBLISHED'){
					myvar +='<input id="learnerscalonly" class="css-checkbox coursecalendersenderstype" type="checkbox" value="LEARNER">'+
					'<label class="css-label pad-left-30 senderseventtype-fontsize" for="learnerscalonly">Learners</label>';
				}
			myvar +='<input id="alumniscalonly" class="css-checkbox coursecalendersenderstype" type="checkbox" value="ALUMNI">'+
			'<label class="css-label pad-left-30 senderseventtype-fontsize" for="alumniscalonly">Alumni</label>'+
			'</div>';
			}

			
			
			el = $("<div id='fc-toolbaridd' class='fc-toolbar'/>")
				//.append(renderSection('left'))
			    .append($('<div class="fc-left" id="categoriesMenuDiv"></div>').append(categoryEle)).off("click").bind("click",function(){
			    	
			    	if(categories && categories.length >= 10){
			    		var xiimcustomScrollbarOptions = {elementid:"#categoriesCalendarUL",isUpdateOnContentResize:true,setHeight:"300px",vertical:'y'};
			    		xiimcustomScrollbar(xiimcustomScrollbarOptions);
					}
			    	
			    	allowClose = false;    
			    	$("#newCategoryEvent").off('click').bind('click',function(e){
		   				var eles = $('.calendarCategories');
		   				var currentCategories = [];
		   				for(var i=0;i<eles.length;i++){
		   					currentCategories.push({
		   						categoryId:$(eles[i]).attr('categoryId'),
		   						categoryName:$(eles[i]).attr('categoryName'),
		   						categoryColorCode:$(eles[i]).attr('categoryColorCode'),
		   						isCustomCategory:$(eles[i]).attr('isCustomCategory')
		   					});
		   				}
			    		var manageCategoryOptions = {
					    		ele:'#manageCalendarCategoriesDiv',
					    		mode:'Manage',
					    		...
					    		onSelect:function(data1){
					    			...
					    			if(categories){
						    			for(var j=0;j<categories.length;j++){
						    				if(categories[j].categoryId != data1.categoryId){
						    					tempCategories.push(categories[j]);
						    				}
						    			}
					    			}
					    			tempCategories.push(data1);
					    			categories = tempCategories;
					    			currentCategories = tempCategories;
					    			/*var newCustomCategoryHtml='<div id="filterCalendarCategoryDiv_'+data1.categoryId+'" categoryId="'+data1.categoryId+'" class="text-left"><input class="css-checkbox" type="checkbox" id="eventCategoryCheckBox_'+data1.categoryId+'" value="'+data1.categoryId+'"><label for="eventCategoryCheckBox_'+data1.categoryId+'" isCustomCategory="'+data1.isCustomCategory+'" categoryColorCode="'+data1.categoryColorCode+'" class="css-label mediumblue" style="color:'+data1.categoryColorCode+'">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+data1.categoryName+'</label></div>';
					    			
	  		                        $("#newCalendarCustomCategoryDiv").before(newCustomCategoryHtml);*/
					    		},
					    		onDelete:function(categoryId){
					    			
					    			var tempCategories = [];
					    			for(var j=0;j<categories.length;j++){
					    				if(categories[j].categoryId != categoryId){
					    					tempCategories.push(categories[j]);
					    				}
					    			}
					    			categories = tempCategories;
					    		...
					    		associationUniqueIdentifier:options.associationUniqueIdentifier
					    };
					    EventCategories.init(manageCategoryOptions);
		   			});
			    	
			    	var isFlag = false;
			    	$("#categoriesMenuDiv").off('click').bind('click',function(e){
			    		if($('#categorydropdownidd').hasClass('selected-sm')){
			    			isFlag = false;
			    			$('#categorydropdownidd').addClass('selected-sm');
			    			$("#categoriesCalendarUL").removeClass('hide');
			    		}else{
			    			isFlag = true;
			    			$('#categorydropdownidd').removeClass('selected-sm');
			    			$("#categoriesCalendarUL").addClass('hide');
			    		}
			    		if(isFlag){
			    			isFlag = false;
		    			$('#categorydropdownidd').addClass('selected-sm');
			    	...
			    	/**
			    	 * This click event will perform the logic between all and other event privacies and invoke service to get new event based on checked event privacies
			    	 */
			    	$('.coursecalendersenderstype').off('click').bind('click',function(e){
			    		var id = $(this).attr('id');
			    		if(id == 'allcalonly'){
			    			//$('.coursecalendersenderstype:not(#allcalonly)').attr('checked',false);
			    			if($(this).is(':checked')){
			    				$('.coursecalendersenderstype:not(#allcalonly)').attr('checked',false);
			    			} else{
			    			...
			    			$('#allcalonly').attr('checked',false);
			    			if(!$('.coursecalendersenderstype').is(':checked')){
			    				$('#allcalonly').prop('checked',true);
			    			}
			    		}
			    		
			    		
			    		//getting all visible checkboxes.
				    	var object = $('.coursecalendersenderstype:visible');
				    	 var checkedCategories = [];
		            	  var checkedDefaultCategories = [];
			    		
		            	  //getting current view of the calendar
		            	  var view = $('#calendar').fullCalendar('getView');
		            	  //getting default request object by passing current view and date.
		            	  var calOptions = FullCalendarWidget.loadEventsRequest(view.name,new Date());
		  					
		  					calOptions.requestInfo['callBack'] = FullCalendarWidget.renderFullCalendarEvents;
		  					var req = $.parseJSON(calOptions.data);
		  					var eventSearchCriteria = req.eventSearchCriteria;
		  					
		  					//here looping with all custom and system default categories and preparing request object.
			            	  $.each($("[id^=eventCategoryCheckBox]"),function(index,value){
			            		  if($(value).is(":checked")){
			            			  if($(value).val() != ""){
			            				  var isCustomCategory = $('#filterCalendarCategoryDiv_'+$(value).val()).attr("isCustomCategory");
			            				  if(isCustomCategory == 'true'){
			            					  checkedCategories.push($(value).val());  
			            				  }else{
			            					  checkedDefaultCategories.push($('#filterCalendarCategoryDiv_'+$(value).val()).attr("categoryType"));
			            				  }
			            	  ...
			            	  for(var i=0;i<checkedCategories.length;i++){
			  						eventSearchCriteria.push({
			  							"eventSearchCriteria": "CATEGORY",
			  							"eventSearchCriteriaValue": checkedCategories[i]
			  						});
		  						}

		  					
		  					for(var i=0;i<checkedDefaultCategories.length;i++){
			  						eventSearchCriteria.push({
			  							"eventSearchCriteria": "EVENTASSOCIATIONTYPE",
			  							"eventSearchCriteriaValue": checkedDefaultCategories[i]
			  						});
		  						}
		  					...
		  					var associationType = $('#associationType').val();//COURSE,GROUP,CONNECTION
		  					//if(associationType == 'CONNECTION'){
			  					eventSearchCriteria.push({
		                            "eventSearchCriteria" : "STATUS",
		                            "eventSearchCriteriaValue" : "PUBLISH"
			    	...
			    	$("body").on('click',function(e){
			    		if(!$(e.target).is("#categorydropdownidd")){
				    		if($("#categorydropdownidd").hasClass('selected-sm')){
				    			$("#categorydropdownidd").removeClass("selected-sm");
				    		}
				    		if($("#categoriesCalendarUL").hasClass('hide')){
				    			$("#categoriesCalendarUL").removeClass("hide");
				    		}
			    		}
			    		if($(e.target).is("#fc-toolbaridd")){
			    			if(!$("#categoriesCalendarUL").hasClass('hide')){
				    			$("#categoriesCalendarUL").addClass("hide");
				    		}
			    		}
			    	...
			    	$("[id^=eventCategoryCheckBox]").off("click").bind("click",function(e){
			    	//	 alert(" category clicked " + $(this).val() ); 
			    		 
		            	  var checkedCategories = [];
		            	  var checkedDefaultCategories = [];

		            	  if($(this).val() == ""){
		            		  $("[id^=eventCategoryCheckBox]").prop('checked', false);
		            		  $("#eventCategoryCheckBox").prop('checked', true);
		            	  }else{
		            	  ...
		            	  var totalCheckedCategories = parseInt(checkedDefaultCategories.length)+parseInt(checkedCategories.length);
		            	  if(totalCheckedCategories == categories.length){
		            		  $("[id^=eventCategoryCheckBox]").prop('checked', false);
		            		  $("#eventCategoryCheckBox").prop('checked', true);
		            	  }else if(totalCheckedCategories == 0){
		            		  $("#eventCategoryCheckBox").prop('checked', true); 
		            	  }else{
		            		  $("#eventCategoryCheckBox").prop('checked', false);
		            	  }
		            	  ...
		            	  var calOptions = FullCalendarWidget.loadEventsRequest(view.name,new Date());
		        		   ...
		        		   $(".white-dropdown-panel span").addClass('selected');
		        		   allowClose = false;
				...
				.append(myvar)//appending the senders type check boxes so that user can filter the events on calendar only on courses.
				//.append(renderSection('center'))
				.append('<div class="fc-clear"/>');
			   
			
               $("#headerDiv").append(renderSection('left'));
               //$("#headerDiv").append(renderSection('center'));
               $("#prevButton").after('<span id="calendarTitleSpan" class="font-24px"></span>');
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							if (customText) {
								...
								innerHtml = "<span class='mar-lr-11 fc-icon right-buttom-toggle " + normalIcon + "'></span>";
							}
							else {
							...
							button = $( // type="button" so that it doesn't submit a form
								/*'<button type="button" id="'+buttonName+'Button" class="' + classes.join(' ') + '">' +
									innerHtml +
								'</button>'*/
									//need to modify here to change the style
									/*Modified by the NextSphere*/
									'<a href="javascript:void(0);" id="'+buttonName+'Button" class="">' +
									innerHtml +
								'&nbsp;  </a>&nbsp;'
								)
								.click(function() {
									...
									if ( $(this).attr("id") != "nextButton" &&
										  $(this).attr("id") != "prevButton"  )
									{
										// only remove selected if date is not changed
										$("[id$='Button']").removeClass('selectedview');	 
									}
	...
	function updateTitle(text) {
		...
		//$('#headerDiv').find('h2').text(text);
		/*Modified by the NextSphere*/
		$("#calendarTitleSpan").html(text);
	}
...
function EventManager(options) { // assumed to be a calendar
	...
	function renderEvent(eventData, stick, report) {
...
function setPotentialScroller(containerEl, height) {
	...
	/**
	 * Xiim changes
	 * Height changed to 100% to fit in the 2x2 view of calendar.
	 */
	containerEl.addClass('fc-scroller').css("height","100%");
...
var tokenOverrides = {
	t: function(date) { // "a" or "p"
		 ...
		 */		
		return ' ' +momentFormat(date, 'AM').substring(0,2);
...
$.extend(DayGrid.prototype, {
	...
	renderSegHtml: function(seg, disableResizing) {
		...
		var timeHtml = '';
		var titleHtml;
		//var dotDiv = '<div style="background-color:'+event.color+'" class="sm-circle-blue"></div>';
		
		classes.unshift('fc-day-grid-event');

		// Only display a timed events time if it is the starting segment
		if (!event.allDay && seg.isStart) {
			timeHtml = '<span class="fc-time calendar-event-font-9px">' + htmlEscape(view.getEventTimeText(event)) + '</span>';
		}

		titleHtml =
			'<span class="fc-title calendar-event-font-9px mar-minus-10">' +
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content"  data-tooltip-toggle="tooltip"  title="'+event.title+'">'  +
					(isRTL ?
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					td = $('<td class="fc-event-container calendar-event-font-9px"/>').append(seg.el);
...
$.extend(DayGrid.prototype, {
	...
	showSegPopover: function(date, cell, moreLink, segs) {
		...
		/** custom code for xiim
		 * to adjust popup position for absolute parenrs
		 */
		var popup_div = $('.fc-more-popover');
		if(popup_div.length){
			var container_div = $('.fc-view-container');
			var max_left = container_div.width();
			var fixed_left = moreLink.position().left+popup_div.width();
			var fixed_top = (moreLink.offset().top - $(document).scrollTop())-moreLink.position().top;
			
			if(fixed_left>max_left){
				popup_div.css('left',max_left+'px');
			}
			else{
				popup_div.css('left',fixed_left+'px');
			}
			popup_div.css('top',fixed_top+'px');
			popup_div.css('position','fixed');		
		}
	},
...
$.extend(TimeGrid.prototype, {
	...
	renderSegHtml: function(seg, disableResizing) {
		...
		return '<a  title="' +htmlEscape(event.title) +'" class="' + classes.join(' ') + '"' +
			(event.url ?
				' href="' + htmlEscape(event.url) + '"' :
				''
				) +
...
function generateAgendaAxisFormat(options, langData) {
	return langData.longDateFormat('LT')
		.replace(':mm', '(:mm)')
		.replace(/(\Wmm)$/, '($1)'); // like above, but for foreign langs
		/** xiim change comment below line */
...
$.extend(AgendaView.prototype, {
	...
	render: function(colCnt) {
		...
		this.bottomRuleEl = $('').appendTo(this.timeGrid.el); // inject it into the time-grid
	...
	renderEvents: function(events) {
		...
		for (i = 0; i < events.length; i++) {
			...
			if(typeof events[i].allDay == 'string'){
...
/*function eventcategorydropdown(e){
	$('#categorydropdownidd').toggleClass('selected-sm');
	$(e.target).parent().parent().toggleClass('open');
	$("#categoriesCalendarUL").toggleClass('hide');
	//$('#categorydropdownidd').attr('data-toggle','dropdown');	
}*/
